...
Cell.prototype.show = function () {
  fill(
    redc[states[this.state]],
    greenc[states[this.state]],
    bluec[states[this.state]]
  );
  ...
  if (borning && this.numX == editingX && this.numY == editingY) {
    ...
    if (cellShape == "square") {
      rect(
        this.posX + edgeThickness,
        this.posY + edgeThickness,
        (cellSize - 3 * edgeThickness) / 2,
    ...
    if (cellShape == "square") {
      rect(
        this.posX + cellSize / 2 + edgeThickness / 2,
        this.posY + edgeThickness,
        (cellSize - 3 * edgeThickness) / 2,
        cellSize - 2 * edgeThickness
      );
  ...
  if (hint || (!hint && this.transitionstate != hintsize)) {
    fill(
      redc[states[this.nextstate]],
      greenc[states[this.nextstate]],
      bluec[states[this.nextstate]]
    );
    var newsize = (this.transitionstate * cellSize) / transitionframes;
    if (cellShape == "square") {
      rectMode(CENTER);
      rect(
        this.posX + cellSize / 2,
        this.posY + cellSize / 2,
        newsize,
        newsize
      );
      rectMode(CORNER);
  ...
  if (hint && this.transitionstate >= hintsize) {
    fill(
      redc[states[this.nextnextstate]],
      greenc[states[this.nextnextstate]],
      bluec[states[this.nextnextstate]]
    );
    var newsize =
      ((this.transitionstate - hintsize) * cellSize) /
      (transitionframes - hintsize) /
      (transitionframes / hintsize);
        ...
        if (ventricle1 == -1) {
          ...
          shapeTransitions.push(
            new ShapeTransition(
              ...
              cellSize,
              cellSize,
              editingX * (cellSize + edgeThickness) +
                cellSize / 2 +
                edgeThickness,
              editingY * (cellSize + edgeThickness) + edgeThickness,
              cellSize / 2,
...
Cell.prototype.checkneighbors = function (defaultstate) {
  var totalNeighbors =
    this.redneighbors + this.blueneighbors + this.grayneighbors;
...
Cell.prototype.clicked = function (x, y) {
  if (
    x >= this.posX &&
    x <= this.posX + cellSize &&
    y >= this.posY &&
    y <= this.posY + cellSize &&
    !this.transitioning
  ) {
    return true;
  } else {

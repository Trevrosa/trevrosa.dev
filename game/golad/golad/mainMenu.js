...
function generateMainMenu() {
  ...
  textAlign(LEFT, CENTER);
  textStyle(ITALIC);
  text("ea!", height / 40, (height * 24) / 25);
...
function generateLogo(alphaValue) {
  ...
  textSize((24 * boardDimension) / 720);
  text("-trevor", width / 6 + height / 7, height / 2 + height / 7);
  textStyle(ITALIC);
  fill(150, alphaValue);
  textAlign(RIGHT, CENTER);
  textSize((22 * boardDimension) / 720);
...
gameButton.prototype.showBtn = function () {
  ...
  if (!this.pressed) {
    ...
    text(
      this.btnName,
      this.btnPosX + this.sizeX / 2,
      this.btnPosY + this.sizeY / 2
    );
  } else {
    fill(0, 60, 55);
    rect(this.btnPosX, this.btnPosY, this.sizeX, this.sizeY, 15);
    ...
    text(
      this.btnName,
      this.btnPosX + this.sizeX / 2,
      this.btnPosY + this.sizeY / 2 + height / 150
    );
  }
};
...
gameButton.prototype.onButton = function (x, y) {
  if (
    x >= this.btnPosX &&
    y >= this.btnPosY &&
    x <= this.btnPosX + this.sizeX &&
    y <= this.btnPosY + this.sizeY
  ) {
    return true;
  } else {
    return false;
...
Picker.prototype.show = function () {
  ...
  rect(
    this.posX + this.sizeX - height / 30,
    this.posY,
    height / 30,
    this.sizeY / 2
  );
  ...
  text(
    "▼",
    this.posX + this.sizeX - height / 60,
    this.posY + (this.sizeY * 3) / 4
  );
};

Picker.prototype.onPress = function (x, y) {
  if (
    x >= this.posX + this.sizeX - height / 30 &&
    y >= this.posY &&
    x <= this.posX + this.sizeX &&
    y < this.posY + this.sizeY / 2
  ) {
    this.up = true;
  }
  if (
...
function nowLoading() {
  if (loadScreen > 0) {
    ...
    var translation =
      ((width / 3) *
        (1 + sin((sin(map(loadScreen, 15, 60, -PI / 2, PI / 2)) * PI) / 2))) /
      2;
    if (loadScreen > 60) {
      translation = width / 3;

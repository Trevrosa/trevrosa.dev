...
$(document).ready(function () {
  ...
  $("#bulk-delete").on("click", () => {
    const confirmation = confirm(
      "Are you sure you want to permanently delete the selected uploads? This action cannot be undone!"
    );
    if (confirmation) {
      const queryParts = [];
  ...
  $("#bulk-download").on("click", () => {
    ...
    window.location.replace(
      `generate_zip_of_files.php?${queryParts.join("&")}`
    );
  });
  ...
  $(".bi-trash-fill").on("click", () => {
    const confirmation = confirm(
      "Are you sure you want to permanently delete all uploads? This action cannot be undone!"
    );
    if (confirmation) {
  ...
  $("#update-button").on("click", () => {
    const confirmation = confirm(
      "Make sure you read the changelog and check for breaking changes before updating.\nAre you sure you would like to update?"
    );
    if (confirmation) {
  ...
  $(".rename-button").on("click", (e) => {
    ...
    if (fileName) {
      ...
      if (newName !== "") {
        window.location.href = `rename_file.php?oldname=${encodeURIComponent(
          fileName
  ...
  $("#file-table").DataTable({
    ...
    language: {
      emptyTable:
        "You have not uploaded any files yet.<br><br>Press the Share icon below to download a ShareX custom uploader configuration file.<br>You can also upload files directly in the box above.",
    },
  });
